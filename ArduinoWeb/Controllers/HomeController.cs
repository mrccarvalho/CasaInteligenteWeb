...
namespace ArduinoWeb.Controllers
{
    public class HomeController : Controller
    {
        ...
        public MedicaoVm MaisRecentes(int relatorioDispositivoId)
        {
            ...
            if (last3.Any())
            {
                var movimento = last3.FirstOrDefault(m => m.TipoMedicaoId == 1);
                

                if (movimento != null)
                {
                    recente.DataMedicao = movimento.DataMedicao;
                    recente.Movimento = movimento.ValorLido;
                }
        ...
        public ActionResult AlteraLocalizacao(int relatorioDispositivoId, int localizacaoId)
        {
            ...
            else if (location == null) { model.Mensagem = $"Location with ID {localizacaoId}  não encontrado"; }
            else
            {
        ...
        /// <returns></returns>
        public ActionResult PostData(int id, string ip, bool movimento)
        {
            var results = "Sucesso";
            ...
            try
            {
                ...
                else
                {
                    ...
                    device.Descricao = "livre";
                    _context.RelatorioDispositivos.Attach(device);
                    _context.Entry(device).Property(x => x.Descricao).IsModified = true;
                   ...
                   // if (movimento)
                    //{
                        ...
                        {
                            TipoMedicaoId = (int)TipoMedicaoEnum.Movimento,
                        ...
                        (from set in recentSet
                         select new GoogleVizDataTable.Row
                         {
                             c = new List<GoogleVizDataTable.Row.RowValue>
                            {
                                ...
                                new GoogleVizDataTable.Row.RowValue { v = set.Movimento },
            ...
            var measureSet =
                (from m in _context.Medicoes.Select(m => m).Include(l => l.Localizacao).AsEnumerable()
                 ...
                 select new MedicaoVm
                 {
                     ...
                     Movimento = g.Where(m => m.TipoMedicaoId == 1).Select(r => r.ValorLido).FirstOrDefault(),
